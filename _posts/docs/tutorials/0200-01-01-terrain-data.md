...
For this tutorial we'll be processing geotiffs on the command-line, so open up your terminal. If you've set up TileMill, the commands we'll be using should already be installed (included in the GDAL library). Run the command `gdaldem --version` to make sure this is the case. It should output something like `GDAL 1.7.3, released 2010/11/10` followed by a bunch of help text.
