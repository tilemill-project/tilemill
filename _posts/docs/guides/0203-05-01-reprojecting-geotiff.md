...
## Reprojecting and Adding GeoTIFFs  
...
2. Run the following command. (*Note that the backslashes are not necessary if you type this all on one line*.)  
><pre>{{page.code1}}</pre>
...
3. Once it is complete, you can load it into your TileMill project. Click **Add Layer** and browse to find the location where you stored `natural-earth-2-mercator.tif`. 
4. Select the file and click **Save and Style**. 
5. To further adjust the resampling interpolation of the image, add the following Carto to your geotiff layer: 
`raster-scaling: bilinear;`  
See the Carto reference manual for additional raster-scaling values. 
