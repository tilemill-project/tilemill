...
On *Ubuntu*, TileMill files are now kept in `~/Documents/MapBox` by default. After starting TileMill for the first time the directory will be created. Move your projects and other data from their old location, such as `/usr/share/mapbox`, to the new directory.

### Execution changes on Ubuntu

TileMill now runs as the logged in user on Ubuntu. You may need to adjust file permissions and ownership on your data files to make them readable by TileMill.
